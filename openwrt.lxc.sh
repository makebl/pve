#!/bin/bash
##########################################################
# URL: https://github.com/makebl/pve
...
URL_Version_Origin="https://raw.githubusercontent.com/makebl/pve/main/lxc/version"
URL_Version_Primary="$Proxy_Primary/$URL_Version_Origin"
URL_Version_Secondary="$CDN_Jsdelivr/makebl/pve/lxc/version"

URL_Script_Origin="https://raw.githubusercontent.com/makebl/pve/main/openwrt.lxc.sh"
URL_Script_Primary="$Proxy_Primary/$URL_Script_Origin"
URL_Script_Secondary="$CDN_Jsdelivr/makebl/pve/openwrt.lxc.sh"
...
function set_github_repository() {
    echo
    read -t 60 -p "请输入仓库地址 [用户名/仓库名, 默认makebl/openwrt]:" input_repo || echo
    input_repo=${input_repo:-"makebl/openwrt"}
...
function linux_uname(){
    if [[ -n `uname -a | grep -i "OpenWrt"` ]]; then
        ...
        echo
        echo "第1步：PVE命令行下载文件"
        __green_color "wget $Proxy_Primary/https://raw.githubusercontent.com/makebl/pve/main/openwrt.lxc.sh -O /usr/bin/openwrt && chmod +x /usr/bin/openwrt"
        echo
        echo "第2步：PVE命令行输入"
